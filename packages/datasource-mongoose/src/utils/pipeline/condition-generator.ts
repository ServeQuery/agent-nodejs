...
export const SERVEQUERY_RECORD_DOES_NOT_EXIST = 'SERVEQUERY_RECORD_DOES_NOT_EXIST';
export default class ConditionGenerator {
  ...
  static tagRecordIfNotExist(field: string, then: AnyExpression): AnyExpression {
    return this.ifMissing(field, then, { SERVEQUERY_RECORD_DOES_NOT_EXIST: true });
  }
  ...
  static tagRecordIfNotExistByValue(field: string, then: AnyExpression): AnyExpression {
    return this.ifMissing(field, then, SERVEQUERY_RECORD_DOES_NOT_EXIST);
  }
