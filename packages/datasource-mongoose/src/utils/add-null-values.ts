import { SERVEQUERY_RECORD_DOES_NOT_EXIST } from './pipeline/condition-generator';

/**
 * Filter out records that have been tagged as not existing
 * If the key SERVEQUERY_RECORD_DOES_NOT_EXIST is present in the record, the record is removed
 * If a nested object has a key with SERVEQUERY_RECORD_DOES_NOT_EXIST, the nested object is removed
 */
function removeNotExistRecord(record: Record<string, unknown>): Record<string, unknown> | null {
  if (!record || record[SERVEQUERY_RECORD_DOES_NOT_EXIST]) return null;

  Object.entries(record).forEach(([key, value]) => {
    if (
      value &&
      typeof value === 'object' &&
      Object.values(value).find(v => v === SERVEQUERY_RECORD_DOES_NOT_EXIST)
    ) {
      record[key] = null;
    }
  });
