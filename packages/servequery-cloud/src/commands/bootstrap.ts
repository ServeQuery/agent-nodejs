...
import {
  ...
  validateMissingServeQueryEnvSecret,
...
export default (program: Command, context: MakeCommands) => {
  ...
  program
    ...
    .option(
      ...
      ' (you can also pass it with environment variable SERVEQUERY_ENV_SECRET)',
    )
    .action(
      actionRunner(logger.spinner, async (folderName: string, options: { envSecret: string }) => {
        ...
        vars.SERVEQUERY_ENV_SECRET = options.envSecret || vars.SERVEQUERY_ENV_SECRET;
        validateMissingServeQueryEnvSecret(vars.SERVEQUERY_ENV_SECRET, 'bootstrap');
