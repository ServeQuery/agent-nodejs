...
export default class HttpServer {
  ...
  constructor(serverUrl: string, secretKey: string, bearerToken: string) {
    ...
    this.headers = {
      'servequery-secret-key': secretKey,
  ...
  async getIntrospection(): Promise<Table[]> {
    return handledAxios(
      ...
      {
        errorMessage: 'Failed to retrieve database schema from Serve Query server',
      },
    );
  }
  ...
  async postUploadRequest(contentLength: number) {
    return handledAxios<{
      ...
      {
        errorMessage: 'Failed to request upload url from Serve Query server',
      },
    );
  }
