...
describe('logs command', () => {
    ...
    it('should display a fail message', async () => {
      ...
      expect(cmd.outputs).toEqual([
        ...
        cmd.spinner.fail(
          ...
          'Your servequery env secret is missing. Please provide it with the `logs --env-secret <your-secret-key>` command or add it to your .env file or in environment variables.',
        ),
        cmd.spinner.stop(),
      ]);
    });
    ...
    it('should display a warning message', async () => {
      ...
      expect(cmd.outputs).toEqual(
        expect.arrayContaining([
          cmd.spinner.warn(
            // eslint-disable-next-line max-len
            'Your version of @servequery/servequery-cloud is outdated. Latest version is 1.0.1.\nPlease update it.',
          ),
        ]),
      );
    });
  ...
  describe('when there are logs', () => {
    ...
    describe('when the to option is given', () => {
      ...
      it.each(['now-1h', 'now-1m', 'now-1s', 'now-1d', 'now-1w', 'now-1M', 'now-1y', 'now'])(
