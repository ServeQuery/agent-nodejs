...
describe('version command', () => {
  ...
  describe('when the major version is greater', () => {
    it('should display a fail', async () => {
      ...
      expect(cmd.outputs).toEqual([
        ...
        cmd.spinner.fail(
          'Your version of @servequery/servequery-cloud is outdated. Latest version is 2.0.0.' +
  ...
  describe('when fails to fetch the version', () => {
    it('should display an info', async () => {
      ...
      expect(cmd.outputs).toEqual([
        cmd.logger.log('1.0.0'),
        cmd.spinner.info('Unable to check the latest version of @servequery/servequery-cloud'),
        cmd.spinner.stop(),
      ]);
    });
  });
