...
describe('bootstrap command', () => {
  ...
  describe('when it is the first time to run bootstrap', () => {
    it('should generate the servequery-cloud folder', async () => {
      const getEnvironmentVariables = jest
        .fn()
        .mockResolvedValueOnce({
          SERVEQUERY_AUTH_TOKEN: null, // auth token is missing because login is required
          ...defaultEnvs,
        })
        .mockResolvedValueOnce({
          SERVEQUERY_AUTH_TOKEN: 'a-token-after-login', // auth token is present after login
          ...defaultEnvs,
        });
      ...
      expect(await fs.readFile(setup.bootstrapPathManager.env, 'utf-8')).toContain(
        'SERVEQUERY_ENV_SECRET=d4ae505b138c30f2d70952421d738627d65ca5322a27431d067479932cebcfa2',
      );
      const generateDotEnv = await fs.readFile(setup.bootstrapPathManager.env, 'utf-8');
      expect(generateDotEnv.replace(/\s/, '')).not.toContain(
        `SERVEQUERY_SERVER_URL=${defaultEnvs.SERVEQUERY_SERVER_URL}`,
      );
    });
  });
    ...
    it('should generate the .env with variables saved', async () => {
      const getEnvironmentVariables = jest
        ...
        .mockResolvedValueOnce({
          SERVEQUERY_AUTH_TOKEN: null, // auth token is missing because login is required
          ...defaultEnvs,
          SERVEQUERY_SERVER_URL: 'https://my-awesome-server.com',
  ...
  describe('when servequery env secret is missing', () => {
    it('should throw an error', async () => {
      ...
      expect(cmd.outputs).toEqual([
        ...
        cmd.spinner.fail(
          ...
          'Your servequery env secret is missing. Please provide it with the `bootstrap --env-secret <your-secret-key>` command or add it to your .env file or in environment variables.',
        ),
        cmd.spinner.stop(),
      ]);
    });
  ...
  describe('when the project has already customization code', () => {
    it('should ask to the customer if he wants to bootstrap again', async () => {
      const getLastPublishedCodeDetails = jest.fn().mockResolvedValue({
        ...
        user: { name: 'John Doe', email: 'johndoad@servequery.com' },
      });
      ...
      expect(cmd.outputs).toEqual([
        ...
        cmd.spinner.info('Last code pushed yesterday, by John Doe (johndoad@servequery.com)'),
        cmd.spinner.warn(
