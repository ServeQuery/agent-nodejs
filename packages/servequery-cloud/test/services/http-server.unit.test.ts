...
describe('http-server', () => {
  ...
  describe('if an error occurs', () => {
    describe('if it is not an axios error', () => {
      it('should throw a business error with details', async () => {
        ...
        await expect(httpServer.getIntrospection()).rejects.toStrictEqual(
          new BusinessError(
            'Failed to retrieve database schema from Serve Query server: Some generic error',
          ),
        );
      });
      ...
      it('should provide specific detail if message contains ERR_INVALID_CHAR', async () => {
        ...
        await expect(httpServer.getIntrospection()).rejects.toStrictEqual(
          new BusinessError(
            `Invalid character in header content ["Authorization"]\nYour authentication token seems incorrect. You can try to login again by running 'npx @servequery/servequery-cloud@latest login'`,
          ),
        );
      });
    });
    ...
    describe('if it an axios error', () => {
      it('should throw a business error with details and inner details', async () => {
        ...
        await expect(httpServer.getIntrospection()).rejects.toStrictEqual(
          new BusinessError(
            'Failed to retrieve database schema from Serve Query server: \n🚨 some details',
          ),
        );
      });
        ...
        it('should throw a Business error with details', async () => {
          ...
          await expect(httpServer.getIntrospection()).rejects.toStrictEqual(
            new BusinessError(
              ...
              "Failed to retrieve database schema from Serve Query server: \n🚨 some details\n You can try to login again by running 'npx @servequery/servequery-cloud@latest login'",
            ),
          );
        });
      });
  ...
  describe('downloadCloudCustomizerTemplate', () => {
    it('should call axios with the correct parameters', async () => {
      ...
      expect(axios.default).toHaveBeenCalled();
      expect(axios.default).toHaveBeenCalledWith({
        url: 'https://github.com/ServeQuery/cloud-customizer/archive/main.zip',
        method: 'get',
        responseType: 'stream',
      });
    });
