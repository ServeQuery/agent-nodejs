import { Table } from '@servequery/datasource-sql';
...
describe('bootstrap', () => {
  const setupMocks = (introspection: Table[]) => {
    ...
    jest.spyOn(fsP, 'readFile').mockImplementation(async () => {
      return `
          env: <SERVEQUERY_ENV_SECRET_TO_REPLACE>
  ...
  describe('If the target directory exists', () => {
    it('should throw a business error', async () => {
      ...
      await expect(
        bootstrap(
          { SERVEQUERY_ENV_SECRET: 'abc' } as unknown as EnvironmentVariables,
          httpServer,
          path,
        ),
      ).rejects.toEqual(new BusinessError('You have already a "servequery-cloud" folder'));
  ...
  describe('If the target directory does not exists', () => {
    describe('If some process fails', () => {
      ...
      describe('If there is an error when trying to clear the bootstrap', () => {
        it('should notify the client to clear the "servequery-cloud" folder', async () => {
          jest.spyOn(fs, 'existsSync').mockReturnValue(false);
          const httpServer = new HttpServer('', '', '');
          const path = new BootstrapPathManager('', '');
          HttpServer.downloadCloudCustomizerTemplate = jest
          ...
          await expect(
            ...
            new BusinessError(
              ...
              'Bootstrap failed: Failed.\nPlease remove "servequery-cloud" folder and re-run bootstrap command.',
            ),
          );
        });
      });
    ...
    describe('If no dependency call fails', () => {
      it('should run the bootstrap completely', async () => {
        ...
        expect(renameSpy).toHaveBeenCalledWith('/tmp/cloud-customizer-main', 'servequery-cloud');
        ...
        expect(firstCallArgs[0]).toBe('servequery-cloud/.env');
        ...
        expect(secondCallArgs[0]).toBe('servequery-cloud/src/index.ts');
