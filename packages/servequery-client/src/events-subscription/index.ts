...
export default class EventsSubscriptionService implements BaseEventsSubscriptionService {
  ...
  async subscribeEvents(): Promise<void> {
    ...
    const eventSourceConfig = {
      // servequery-secret-key act as the credential
      withCredentials: false,
      headers: { 'servequery-secret-key': this.options.envSecret },
      https: {
        rejectUnauthorized: process.env.NODE_TLS_REJECT_UNAUTHORIZED !== '0',
      },
    };
    const url = new URL('/liana/v4/subscribe-to-events', this.options.servequeryServerUrl).toString();
  ...
  private onEventError(event: { type: string; status?: number; message?: string }) {
    ...
    if (status === 404) {
      throw new Error(
        'Serve Query server failed to find the environment ' +
  ...
  private onEventOpenAgain() {
    this.options.logger(
      'Debug',
      'Server Event - Open EventSource (SSE) connection with Serve Query servers',
    );
