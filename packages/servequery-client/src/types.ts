...
import { HttpOptions } from './permissions/servequery-http-api';
import {
...
export type ServeQueryClientOptions = {
  envSecret: string;
  servequeryServerUrl?: string;
  logger?: Logger;
...
export type ServeQueryClientOptionsWithDefaults = Required<
  Omit<ServeQueryClientOptions, 'experimental'>
> &
  Pick<ServeQueryClientOptions, 'experimental'>;

export type ServeQueryAuthServiceInterface = {
  init: () => Promise<void>;
...
export interface ServeQueryClient {
  ...
  readonly authService: ServeQueryAuthServiceInterface;

  verifySignedActionParameters<TSignedParameters>(signedParameters: string): TSignedParameters;

  getIpWhitelistConfiguration(): Promise<IpWhitelistConfiguration>;

  postSchema(schema: ServeQuery): Promise<boolean>;
...
export interface ServeQueryServerInterface {
  ...
  makeAuthService(options: ServeQueryClientOptionsWithDefaults): ServeQueryAuthServiceInterface;
}
