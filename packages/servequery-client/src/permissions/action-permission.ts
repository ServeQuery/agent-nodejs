...
import type { ServeQueryClientOptionsWithDefaults } from '../types';

import generateActionsFromPermissions, {
  ActionPermissions,
} from './generate-actions-from-permissions';
import { ServeQueryServerInterface } from '../types';
...
export default class ActionPermissionService {
  ...
  private async fetchEnvironmentPermissions(): Promise<ActionPermissions> {
    ...
    const rawPermissions = await this.servequeryAdminServerInterface.getEnvironmentPermissions(
      this.options,
    );
