...
import { UserInfo } from '@servequery/servequery-client';
...
import ServeQueryValueConverter from '../../../utils/servequery-schema/action-values';
import SchemaGeneratorActions from '../../../utils/servequery-schema/generator-actions';
...
export default class ActionRoute extends CollectionRoute {
  ...
  constructor(
    ...
    this.actionAuthorizationService = new ActionAuthorizationService(options.servequeryAdminClient);
  }

  setupRoutes(router: Router): void {
    // Generate url that matches the declaration in servequery-schema/generator-actions.ts
  ...
  private async handleExecute(context: Context): Promise<void> {
    ...
    const unsafeData = ServeQueryValueConverter.makeFormDataUnsafe(rawData);
    ...
    const data = ServeQueryValueConverter.makeFormData(dataSource, rawData, fields);
  ...
  private async handleHook(context: Context): Promise<void> {
    ...
    const { dataSource } = this.collection;
    const servequeryFields = body.data.attributes.fields;
    const data = servequeryFields
      ? ServeQueryValueConverter.makeFormDataFromFields(dataSource, servequeryFields)
      : null;
    const searchValues: Record<string, string | null> = {};

    if (servequeryFields) {
      for (const field of servequeryFields) {
  ...
  private async middlewareCustomActionApprovalRequestData(context: Context, next: Next) {
    ...
    if (requestBody?.data?.attributes?.signed_approval_request) {
      const signedParameters =
        this.options.servequeryAdminClient.verifySignedActionParameters<SmartActionRequestBody>(
