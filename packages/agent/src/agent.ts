...
import {
  ...
  TCollectionName,
  TSchema,
} from '@servequery/datasource-customizer';
import { DataSource, DataSourceFactory } from '@servequery/datasource-toolkit';
...
import SchemaGenerator from './utils/servequery-schema/generator';
import OptionsValidator from './utils/options-validator';

/**
 * Allow to create a new Serve Query agent from scratch.
...
export default class Agent<S extends TSchema = TSchema> extends FrameworkMounter {
  ...
  async start(): Promise<void> {
    ...
    await this.options.servequeryAdminClient.subscribeToServerEvents();
    this.options.servequeryAdminClient.onRefreshCustomizations(this.restart.bind(this));
  ...
  override async stop(): Promise<void> {
    // Close anything related to ServeQuery client
    this.options.servequeryAdminClient.close();
   ...
   * @see {@link https://docs.servequery.com/developer-guide-agents-nodejs/data-sources/connection Documentation Link}
   */
   ...
   * @see {@link https://docs.servequery.com/developer-guide-agents-nodejs/getting-started/install/autocompletion-and-typings Documentation Link}
   */
   ...
   * @see {@link https://docs.servequery.com/developer-guide-agents-nodejs/agent-customization/agent-customization Documentation Link}
   * @example
   ...
   * @example
   * import advancedExportPlugin from '@servequery/plugin-advanced-export';
   *
  ...
  /**
   * Send the apimap to servequery admin server
   */
  private async sendSchema(dataSource: DataSource): Promise<void> {
    ...
    let schema: Pick<ServeQuery, 'collections'>;
    ...
    // Send schema to servequery servers
    await this.options.servequeryAdminClient.postSchema({ ...schema, meta });
  }
}
