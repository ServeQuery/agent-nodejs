...
type SerializedRecord = { servequeryId: string };
...
export default class Serializer {
  ...
  private registerCollection(collection: Collection, serializer: JsonApiSerializer): void {
    serializer.register(collection.name, {
      ...
      beforeSerialize: (data: Record<string, unknown>) => {
        const copy = { ...data };
        copy.servequeryId = IdUtils.packId(collection.schema, data);
      ...
      afterDeserialize: (data: Record<string, unknown>) => {
        ...
        if (data.servequeryId) {
          const parts = IdUtils.unpackId(collection.schema, data.servequeryId as string);
  ...
  private buildRelationshipsConfiguration(
    ...
    const urlPrefix = path.posix.join('/servequery', collection.name);
      ...
      if (field.type === 'ManyToMany' || field.type === 'OneToMany') {
        relationships[name] = {
          ...
          links: (data: SerializedRecord) => ({
            related: {
              href: `${urlPrefix}/${data.servequeryId}/relationships/${name}`,
            },
          }),
        };
      }
