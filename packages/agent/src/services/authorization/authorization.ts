import { Collection, ConditionTree, UnprocessableError } from '@servequery/datasource-toolkit';
import {
...
export default class AuthorizationService {
  ...
  private async assertCanOnCollection(
    ...
    const canOnCollection = await this.servequeryAdminClient.permissionService.canOnCollection({
      userId,
      event,
      collectionName,
    });
  ...
  public async assertCanExecuteChart(context: Context): Promise<void> {
    ...
    try {
      const canRetrieve = await this.servequeryAdminClient.permissionService.canExecuteChart({
        renderingId,
        userId,
        chartRequest,
      });
  ...
  public async getScope(collection: Collection, context: Context): Promise<ConditionTree> {
    ...
    const scope = await this.servequeryAdminClient.getScope({
  ...
  public invalidateScopeCache(renderingId: number | string) {
    this.servequeryAdminClient.markScopesAsUpdated(renderingId);
  }
}
