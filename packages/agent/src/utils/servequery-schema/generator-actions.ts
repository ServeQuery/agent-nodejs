...
import { ServeQueryServerAction, ServeQueryServerActionField } from '@servequery/servequery-client';
import path from 'path';

import ActionFields from './action-fields';
import ServeQueryValueConverter from './action-values';
import GeneratorActionFieldWidget from './generator-action-field-widget';

export default class SchemaGeneratorActions {
  /**
   * 'fields' sent to servequery-server when we want to generate the form on demand.
   * This works around a bug in frontend which won't call the server if no fields are defined.
   */
  static defaultFields: ServeQueryServerActionField[] = [
    {
      field: 'Loading...',
      type: 'String',
      isReadOnly: true,
  ...
  static async buildSchema(collection: Collection, name: string): Promise<ServeQueryServerAction> {
    ...
    return {
      ...
      baseUrl: null,
      endpoint: path.posix.join('/servequery/_actions', collection.name, String(actionIndex), slug),
      httpMethod: 'POST',
  ...
  static buildFieldSchema(dataSource: DataSource, field: ActionField): ServeQueryServerActionField {
    ...
    output.field = label;
    output.value = ServeQueryValueConverter.valueToServeQuery(field, field.value);
    ...
    return output as ServeQueryServerActionField;
  }
  ...
  private static async buildFields(
    ...
    name: string,
    schema: ActionSchema,
  ): Promise<ServeQueryServerActionField[]> {
