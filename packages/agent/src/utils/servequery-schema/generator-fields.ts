...
import { ServeQueryServerColumnType, ServeQueryServerField } from '@servequery/servequery-client';
...
export default class SchemaGeneratorFields {
  private static relationMap: Partial<Record<FieldTypes, ServeQueryServerField['relationship']>> = {
  ...
  static buildSchema(collection: Collection, name: string): ServeQueryServerField {
    const { type } = collection.schema.fields[name];

    let schema: ServeQueryServerField;
  ...
  private static buildColumnSchema(collection: Collection, name: string): ServeQueryServerField {
  ...
  private static convertColumnType(type: ColumnType): ServeQueryServerColumnType {
  ...
  private static buildToManyRelationSchema(
    ...
    baseSchema: ServeQueryServerField,
  ...
  private static buildRelationSchema(collection: Collection, name: string): ServeQueryServerField {
