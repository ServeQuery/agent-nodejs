import createServeQueryClient from '@servequery/servequery-client';
...
export default class OptionsValidator {
  ...
  static withDefaults(options: AgentOptions): AgentOptionsWithDefaults {
    ...
    copyOptions.schemaPath = copyOptions.schemaPath || '.servequery-schema.json';
    copyOptions.servequeryServerUrl = copyOptions.servequeryServerUrl || 'https://api.servequery.com';
    ...
    copyOptions.servequeryAdminClient =
      copyOptions.servequeryAdminClient ||
      createServeQueryClient({
        envSecret: copyOptions.envSecret,
        servequeryServerUrl: copyOptions.servequeryServerUrl,
  ...
  static validate(options: AgentOptions): AgentOptionsWithDefaults {
    OptionsValidator.checkServeQueryServerOptions(options);
  ...
  private static checkServeQueryServerOptions(options: AgentOptions): void {
    if (typeof options.envSecret !== 'string' || !/^[0-9a-f]{64}$/.test(options.envSecret)) {
      throw new Error(
        ...
        'https://www.servequery.com',
      );
    }

    if (!OptionsValidator.isUrl(options.servequeryServerUrl)) {
      throw new Error(
        'options.servequeryServerUrl is invalid. It should contain an URL ' +
        '(i.e. "https://api.servequery.com")',
      );
    }
    ...
    if (!OptionsValidator.isExistingPath(options.schemaPath)) {
      throw new Error(
        ...
        'where the schema should be loaded/updated (i.e. "./.servequery-schema.json")',
      );
    }
