...
describe('Builder > Agent', () => {
  describe('standalone mode', () => {
    describe('when starting the agent is a success', () => {
      it('should send a response', async () => {
        ...
        try {
          ...
          const response = await superagent.get('http://localhost:9997/my-api/servequery');
    ...
    describe('when the agent is not started', () => {
      it('should throw an error', async () => {
        ...
        await expect(() => superagent.get('http://localhost:9994/my-api/servequery')).rejects.toThrow();
        await mounter.stop();
      });
    });
  ...
  describe('when agent is mounted without the standalone mode or koa', () => {
    describe('when the agent is not started', () => {
      it('should return a default response "Agent is not started"', async () => {
        ...
        try {
          const response = await superagent.get('http://localhost:9998/my-api/servequery');
    ...
    describe('when the agent is restarted', () => {
      it('should unmount previous routes and mount new ones', async () => {
        ...
        try {
          ...
          const newResponse = await superagent.get('http://localhost:9998/my-api/servequery/resource');
          expect(newResponse.body).toStrictEqual({ message: 'Here is your resource' });

          // The previous route has been unmounted
          await expect(superagent.get('http://localhost:9998/my-api/servequery')).rejects.toThrow(
            'Not Found',
          );
        } finally {
          server.close();
  ...
  it.each([
    ...
    try {
      const response = await superagent.get('http://localhost:9999/my-api/servequery');
      expect(response.body).toStrictEqual({ error: null, message: 'Agent is running' });
    } finally {
      app.close();
    }
  ...
  it('should work in NestJS/Express', async () => {
    ...
    try {
      const response = await superagent.get('http://localhost:9996/my-api/servequery');
      expect(response.body).toStrictEqual({ error: null, message: 'Agent is running' });
    } finally {
      await app.close();
    }
  ...
  it('should work in NestJS/Fastify', async () => {
    ...
    try {
      const response = await superagent.get('http://localhost:9995/my-api/servequery');
      expect(response.body).toStrictEqual({ error: null, message: 'Agent is running' });
    } finally {
      await app.close();
    }
