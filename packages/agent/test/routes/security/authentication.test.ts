...
describe('Authentication', () => {
  ...
  beforeEach(async () => {
    ...
    options = factories.servequeryAdminHttpDriverOptions.build();
  ...
  test('bootstrap should retrieve the openId client', async () => {
    const init = jest.spyOn(options.servequeryAdminClient.authService, 'init').mockResolvedValue();
  ...
  describe('when the route is bootstraped', () => {
    describe('/authentication', () => {
      test('responds with auth url if renderingId is correct', async () => {
        ...
        jest
          .spyOn(options.servequeryAdminClient.authService, 'generateAuthorizationUrl')
    ...
    describe('/authentication/callback', () => {
      test('responds with token if auth is successful', async () => {
        ...
        jest.spyOn(options.servequeryAdminClient.authService, 'generateTokens').mockResolvedValue({
          accessToken: '123',
        });

        jest.spyOn(options.servequeryAdminClient.authService, 'getUserInfo').mockResolvedValue({
          id: 1,
          email: 'hello@servequery.admin',
          firstName: 'erlich',
          lastName: 'bachman',
          team: 'admin',
          renderingId: 1,
      ...
      test('throws if it fails to fetch userinfo (exception)', async () => {
        ...
        jest
          .spyOn(options.servequeryAdminClient.authService, 'getUserInfo')
      ...
      test('throws if it fails to fetch userinfo (null)', async () => {
        options.servequeryAdminClient.authService.getUserInfo = jest.fn().mockResolvedValue(null);
      ...
      describe('exception handling', () => {
        test('returns a translated error if it is an AuthenticationError', async () => {
          ...
          jest.spyOn(options.servequeryAdminClient.authService, 'generateTokens').mockRejectedValue(
        ...
        test('rethrow the error if it is not an AuthenticationError', async () => {
          ...
          jest
            .spyOn(options.servequeryAdminClient.authService, 'generateTokens')
