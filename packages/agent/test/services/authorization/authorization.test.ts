...
describe('AuthorizationService', () => {
  ...
  describe.each([
    ...
    it('should not do anything for authorized users', async () => {
      const servequeryAdminClient = factories.servequeryAdminClient.build();
      const authorizationService = new AuthorizationService(servequeryAdminClient);
      ...
      (servequeryAdminClient.permissionService.canOnCollection as jest.Mock).mockResolvedValue(true);

      await authorizationService[assertion](context, 'books');

      expect(context.throw).not.toHaveBeenCalled();

      expect(servequeryAdminClient.permissionService.canOnCollection).toHaveBeenCalledWith({
        userId: 35,
        event,
    ...
    it('should throw an error when the user is not authorized', async () => {
      ...
      (servequeryAdminClient.permissionService.canOnCollection as jest.Mock).mockResolvedValue(false);
  ...
  describe('getScope', () => {
    it('should return the scope for the given user', async () => {
      ...
      (servequeryAdminClient.getScope as jest.Mock).mockResolvedValue({ foo: 'bar' });
      ...
      expect(servequeryAdminClient.getScope).toHaveBeenCalledWith({
        renderingId: 42,
        userId: 666,
        collectionName: 'books',
      });
  ...
  describe('assertCanExecuteChart', () => {
    it('should check if the user can retrieve the chart and do nothing if OK', async () => {
      ...
      (servequeryAdminClient.permissionService.canExecuteChart as jest.Mock).mockResolvedValue(true);

      await authorizationService.assertCanExecuteChart(context);

      expect(context.throw).not.toHaveBeenCalled();

      expect(servequeryAdminClient.permissionService.canExecuteChart).toHaveBeenCalledWith({
        renderingId: 42,
        userId: 35,
    ...
    it('should throw an error if the user cannot retrieve the chart', async () => {
      ...
      (servequeryAdminClient.permissionService.canExecuteChart as jest.Mock).mockResolvedValue(false);
    ...
    it('should throw an error if the query is empty', async () => {
      ...
      (servequeryAdminClient.permissionService.canExecuteChart as jest.Mock).mockRejectedValue(
  ...
  describe('invalidateScopeCache', () => {
    it('should invalidate the scope cache', () => {
      ...
      expect(servequeryAdminClient.markScopesAsUpdated).toHaveBeenCalledWith(42);
    });
  });
});
