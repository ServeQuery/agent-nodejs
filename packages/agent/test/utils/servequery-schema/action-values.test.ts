import ServeQueryValueConverter from '../../../src/utils/servequery-schema/action-values';
import * as factories from '../../__factories__';

describe('ServeQueryValueConverter', () => {
  ...
  describe('makeFormData', () => {
    test('should ignore the special "Loading" field', () => {
      ...
      const formData = ServeQueryValueConverter.makeFormData(dataSource, data, fields);
  ...
  describe('makeFormDataFromFields', () => {
    test('should ignore the special "Loading" field', () => {
      ...
      const formData = ServeQueryValueConverter.makeFormDataFromFields(dataSource, fields);
  ...
  describe('makeFormDataUnsafe', () => {
    test('should ignore the special "Loading" field', () => {
      const data = { 'Loading...': 'hello' };
      const formData = ServeQueryValueConverter.makeFormDataUnsafe(data);
  ...
  describe('valueToServeQuery', () => {
    test('should check enums', () => {
      ...
      expect(ServeQueryValueConverter.valueToServeQuery(field, 'd')).toEqual(null);
      expect(ServeQueryValueConverter.valueToServeQuery(field, 'c')).toEqual('c');
    });
    ...
    test('should check enum lists', () => {
      ...
      expect(ServeQueryValueConverter.valueToServeQuery(field, ['d', 'c'])).toEqual(['c']);
      expect(ServeQueryValueConverter.valueToServeQuery(field, ['c'])).toEqual(['c']);
    });
    ...
    test('should pack ids', () => {
      ...
      expect(ServeQueryValueConverter.valueToServeQuery(field, ['3', 'aa'])).toEqual('3|aa');
    });
    ...
    test('should make data uris', () => {
      ...
      expect(
        ServeQueryValueConverter.valueToServeQuery(field, {
          mimeType: 'text/csv',
