...
import { DataSourceCustomizer } from '@servequery/datasource-customizer';
...
jest.mock('@servequery/datasource-customizer', () => ({
  __esModule: true,
...
describe('Agent', () => {
  describe('Development', () => {
    const options = factories.servequeryAdminHttpDriverOptions.build({
      ...
      servequeryAdminClient: factories.servequeryAdminClient.build({ postSchema: mockPostSchema }),
    });
    ...
    test('start should subscribe server events and add listener to restart', async () => {
      ...
      expect(options.servequeryAdminClient.subscribeToServerEvents).toHaveBeenCalledTimes(1);
      expect(options.servequeryAdminClient.onRefreshCustomizations).toHaveBeenCalledTimes(1);
    });
  ...
  describe('stop', () => {
    test('stop should close the Serve Query client', async () => {
      const options = factories.servequeryAdminHttpDriverOptions.build();
      const agent = new Agent(options);

      await agent.stop();

      expect(options.servequeryAdminClient.close).toHaveBeenCalledTimes(1);
    });
  });
